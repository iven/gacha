...
你可以在紧凑的刘海界面里复习卡片。Gacha 使用 FSRS 记忆调度模型，根据你自己的复习历史形成个性化记忆曲线，并决定每张卡片在记忆变模糊前什么时候再次出现。

## 功能

- 支持 Markdown 的记忆卡片，并支持 ruby 注音。
- 卡片和记忆状态以纯文本存放，可配合自己的文件同步工具在多台机器间同步。
- 基于刘海的复习流程，支持键盘快捷键。
- 基于 FSRS v6 的间隔重复调度。
...
- 不支持 Markdown 图片、表格和代码高亮。
- 不支持 CSV 和 Anki 卡片导入；可以借助外部 AI Agent 将这些来源整理成 Gacha 可用的 Markdown 卡片。
