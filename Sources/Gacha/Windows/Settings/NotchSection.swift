...
struct NotchSection: View {
  ...
  let onPresentationPolicyChanged: () -> Void

  private static let memoryCardAutoCollapsePresetSeconds = [
    0, 1, 2, 3, 5, 10, 15, 30, 60,
  ]
  private static let noticeAutoCollapsePresetSeconds = Array(1...10)
  ...
  @State private var noticeAutoCollapseSeconds: Int
  ...
  init(settingsStore: SettingsStore, onPresentationPolicyChanged: @escaping () -> Void) {
    self.settingsStore = settingsStore
    self.onPresentationPolicyChanged = onPresentationPolicyChanged
    _memoryCardAutoCollapseSeconds = State(
      initialValue: Int(settingsStore.memoryCardAutoCollapseSeconds))
    _noticeAutoCollapseSeconds = State(
      initialValue: Int(settingsStore.noticeAutoCollapseSeconds))
  ...
  var body: some View {
    Section(SettingsStrings.sectionNotch) {
      ...
      LabeledContent(SettingsStrings.noticeAutoCollapse) {
        Picker("", selection: noticeAutoCollapseSecondsBinding) {
          ForEach(Self.noticeAutoCollapsePresetSeconds, id: \.self) { seconds in
            Text("\(seconds) \(SettingsStrings.noticeAutoCollapseUnit)")
              .tag(seconds)
          }
        }
        .labelsHidden()
  ...
  private var noticeAutoCollapseSecondsBinding: Binding<Int> {
    Binding(
      get: { noticeAutoCollapseSeconds },
      set: { newValue in
        settingsStore.noticeAutoCollapseSeconds = TimeInterval(newValue)
        noticeAutoCollapseSeconds = Int(settingsStore.noticeAutoCollapseSeconds)
        onPresentationPolicyChanged()
      })
  }
